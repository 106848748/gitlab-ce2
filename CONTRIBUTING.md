...
~issues, ~"merge request", ~labels, and ~"container registry".
...
- We advertise `Accepting Merge Requests` issues with weight < 5 as suitable for
  people that have never contributed to GitLab before on the
  [Up For Grabs campaign](http://up-for-grabs.net)
- We encourage people that have never contributed to any open source project to
  look for ~"Accepting Merge Requests" issues with a weight of 1
