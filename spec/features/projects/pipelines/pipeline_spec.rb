...
describe 'Pipeline', :feature, :js do
  ...
  describe 'GET /:project/pipelines/:id/failures' do
    let(:project) { create(:project) }
    let(:pipeline) { create(:ci_pipeline, project: project, ref: 'master', sha: project.commit.id) }
    let(:pipeline_failues_page) { failures_namespace_project_pipeline_path(project.namespace, project, pipeline) }
    let!(:failed_build) { create(:ci_build, :failed, pipeline: pipeline) }

    context 'with failed build' do
      before do
        failed_build.trace.set('4 examples, 1 failure')
        visit pipeline_failues_page
      end
    ...
    context 'when missing build logs' do
      ...
      it 'includes failed jobs' do
        expect(page).to have_content('No build trace')
      end
    end
  end
end
