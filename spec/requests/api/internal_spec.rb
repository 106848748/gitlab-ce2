...
describe API::Internal do
    ...
    context "access granted" do
      ...
      context "git push with project.wiki" do
        it 'responds with success' do
          ...
          expect(json_response["gl_project"]).to eq("wiki-#{project.id}")
      ...
      context "git pull" do
        it do
          ...
          expect(json_response["gl_project"]).to eq("project-#{project.id}")
          expect(user).to have_an_activity_record
        end
      end
  ...
  describe 'GET /internal/merge_request_urls' do
    ...
    context 'with a gl_project parameter' do
      let(:gl_project) { "project-#{project.id}" }

      it 'returns link to create new merge request' do
        get api("/internal/merge_request_urls?gl_project=#{gl_project}&changes=#{changes}"), secret_token: secret_token
  ...
  describe 'POST /notify_post_receive' do
    ...
    context 'with a gl_project parameter' do
      let(:valid_params) do
        { gl_project: "project-#{project.id}", secret_token: secret_token }
      end

      let(:valid_wiki_params) do
        { gl_project: "wiki-#{project.id}", secret_token: secret_token }
      end
